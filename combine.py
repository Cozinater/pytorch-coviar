...
def main():
    ...
    parser.add_argument('--iframe', type=str,
    ...
    parser.add_argument('--noiframe', type=bool,
    help='only the score of mv and residual is combined. iframe is not included')
    ...
    if (args.noiframe):
        with np.load(args.mv, allow_pickle=True) as mv:
            with np.load(args.res, allow_pickle=True) as residual:
                ...
                assert np.alltrue(res_label == mv_label)
                combined_score =  mv_score * args.wm + res_score * args.wr

                accuracy = float(sum(np.argmax(combined_score, axis=1) == mv_label)) / n
